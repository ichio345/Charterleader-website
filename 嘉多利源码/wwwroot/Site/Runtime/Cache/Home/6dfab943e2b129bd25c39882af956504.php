...
<div class="wrap">
       <ul class="ny-proclass">
          <?php $db=M("class"); $where=array('PID'=>264); $data=$db->where($where)->limit(50)->order('sort')->select(); foreach ($data as $k=>$v): $classify['title']=substr_cut($v['Name'],max); $classify['link']=get_url($v['ID']); $picpath='Upload'.$v['Class_pic']; if(!empty($v['Class_pic']) && file_exists($picpath)){ $classify['src']='/Upload'.$v['Class_pic']; }else{ $classify['src']='/Common/images/not-pic.jpg'; } $classify['desc']=substr_cut($v['Class_info'],max); $noid=$v['ID']; ?>
           <li>
               <div class="title">
                   <h2><a href="<?php echo ($classify["link"]); ?>"><?php echo ($classify["title"]); ?></a></h2>
                   <dl>
                       <?php $db=M("class"); $where=array('PID'=>$noid); $data=$db->where($where)->limit(10)->order('sort')->select(); foreach ($data as $k=>$vv): $classifyy['title']=substr_cut($vv['Name'],max); $classifyy['link']=get_url($vv['ID']); $picpath='Upload'.$vv['Class_pic']; if(!empty($vv['Class_pic']) && file_exists($picpath)){ $classifyy['src']='/Upload'.$vv['Class_pic']; }else{ $classifyy['src']='/Common/images/not-pic.jpg'; } $classifyy['desc']=substr_cut($vv['Class_info'],max); ?><dd><a href="<?php echo ($classifyy["link"]); ?>"><?php echo ($classifyy["title"]); ?></a><em>/</em></dd><?php endforeach;?>
                   </dl>
               </div>
               <div class="content">
                   <div class="detail bg2">
                     <!--   <h3><?php echo ($classify["title"]); ?></h3> -->
                       <p><?php echo ($classify["desc"]); ?></p>
                       <a href="<?php echo ($classify["link"]); ?>" class="more">More+</a>
                       <img src="<?php echo ($classify["src"]); ?>" height="135" width="190">
                   </div>   
                   <ul class="ny-latest">
                      <?php $where=""; $where2=array( "path"=>array('like',"%$noid%") ,"Edition"=>C('EDITION') ,"is_index"=>1 ); if(!myempty($where)){ $data=D("Product")->where($where2)->where($where)->limit(10)->order("Sort DESC,ID DESC")->relation('content_on_type')->select(); }else{ $data=D("Product")->where($where2)->limit(10)->order("Sort DESC,ID DESC")->relation('content_on_type')->select(); } foreach ($data as $product) : $piclist=explode("###",$product['Thumbnail'])[$product['Cover']]; $picpath='Upload'.$piclist; if(!empty($piclist) && file_exists($picpath)){ $product["src"]='/Upload'.$piclist; }else{ $product["src"]='/Common/images/not-pic.jpg'; } $product["title"]=substr_cut($product["Title"],18); $product["desc"]=substr_cut($product["Description"],max); $product["keyword"]=substr_cut($product["Keyword"],max); $product["link"]=get_url($product['ClassID'],$product["ID"]); $product["id"]=$product["ID"]; foreach ($product["content_on_type"] as $key => $value) { $product[$value['typeremark']]=$value['typevalue']; } ?><li>
                           <a href="<?php echo ($product["link"]); ?>">
                               <div class="pic">
                                   <img src="<?php echo ($product["src"]); ?>" height="182" width="230" alt="<?php echo ($product["title"]); ?>" title="<?php echo ($product["title"]); ?>">
                               </div>
...
<div class="clear"></div><?php endforeach;?>
       </ul>
    </div>
