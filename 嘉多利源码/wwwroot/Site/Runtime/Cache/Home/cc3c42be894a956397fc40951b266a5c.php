...
<div class="wrap">
       <ul class="ny-proclass">
          <?php $db=M("class"); $where=array('PID'=>240); $data=$db->where($where)->limit(50)->order('sort')->select(); foreach ($data as $k=>$v): $classify['title']=substr_cut($v['Name'],max); $classify['link']=get_url($v['ID']); $picpath='Upload'.$v['Class_pic']; if(!empty($v['Class_pic']) && file_exists($picpath)){ $classify['src']='/Upload'.$v['Class_pic']; }else{ $classify['src']='/Common/images/not-pic.jpg'; } $classify['desc']=substr_cut($v['Class_info'],max); $noid=$v['ID']; ?>
           <li>
               <div class="title">
