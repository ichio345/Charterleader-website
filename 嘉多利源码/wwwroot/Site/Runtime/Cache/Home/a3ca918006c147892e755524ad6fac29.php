...
<div class="ny-content">
    ...
    <div class="jdl4">
        ...
        <div class="jianying">
            <ul class="swiper-wrapper">
                <?php
 $where=""; $where=array( "path"=>array('like',"%315%") ,"Edition"=>C('EDITION') ,"is_index"=>1 ); $data=M('pic')->where($where)->limit(10)->select(); foreach ($data as $pic) : $pic['title']=substr_cut($pic['Title'],max); $pic['desc']=substr_cut($pic['Description'],max); $pic['auth']=substr_cut($pic['Auth'],max); $piclist=explode("###",$pic['Thumbnail'])[$pic['Cover']]; $picpath='Upload'.$piclist; if(!empty($piclist) && file_exists($picpath)){ $pic["src"]='/Upload'.$piclist; }else{ $pic["src"]='/Common/images/not-pic.jpg'; } $pic['link']=get_url($pic['ClassID'],$pic['ID']); ?><li class="swiper-slide"><img src="<?php echo ($pic["src"]); ?>"></li><?php endforeach; ?>
            </ul>
