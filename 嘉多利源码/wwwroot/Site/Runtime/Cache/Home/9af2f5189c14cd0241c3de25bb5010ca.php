...
<?php if($_GET['cid'] == 244 or $_GET['cid'] == 309 ): else: ?>
    <div class="ny-column">
        <div class="wrap">
            <h2><?php $bname["name"]="产品中心"; $bname["remark"]="PRODUCT CENTER"; $bname["src"]="/Upload"; echo ($bname["name"]); ?></h2>
            <p><?php $bname["name"]="产品中心"; $bname["remark"]="PRODUCT CENTER"; $bname["src"]="/Upload"; echo ($bname["remark"]); ?></p>
        </div>
    </div><?php endif; ?>
...
<div class="ny-content">
	...
	<div class="wrap">
        <!-- pc 左侧 begin -->
        <div class="ny-left pc">
            <div class="title">
                <h2><?php $bname["name"]="产品中心"; $bname["remark"]="PRODUCT CENTER"; $bname["src"]="/Upload"; echo ($bname["name"]); ?></h2>
                <em><?php $bname["name"]="产品中心"; $bname["remark"]="PRODUCT CENTER"; $bname["src"]="/Upload"; echo ($bname["remark"]); ?></em>
            </div>
            <?php $show="now_all";$id="";$type="product";$db=M("class"); $isid=!myempty($id)?$id:I('cid'); $ModelID=M("model")->field("ID")->where("Name='product'")->find(); $column=blnav(); if(!empty($type)){ $where=array('ModelID'=>$ModelID['ID'],'Child'=>array('neq',0),'Status'=>1,'Edition'=>C('edition')); }else{ $where=array('Child'=>array('neq',0),'Status'=>1,'Edition'=>C('edition')); } if($show=="all"){ !empty($id)?$where['ID']=$id:''; }else if($show=="now"){ $where['ID']=$isid; }else if($show=='now_all'){ $gid=$_GET['cid']; $data=$db->field('path')->where('ID='.$gid.'')->find(); !empty($data['path'])?$path=explode(',', $data['path'])[0]:$gid; $where['ID']=$path; } $pid=$db->where($where)->order('depth')->find(); empty($pid['ID'])?$pid['ID']=I('cid'):$pid['ID']; import("Class.expand",APP_PATH); $object = new \Expand(); $column=$object->erwei2($column,'',$pid['ID']); echo classNav($column); ?>		
        </div>
        <!-- pc 左侧 end -->

        <!-- wap 左侧 begin -->
        <div class="ny-left wap">
               <?php $show="now_all";$id="";$type="product";$db=M("class"); $isid=!myempty($id)?$id:I('cid'); $ModelID=M("model")->field("ID")->where("Name='product'")->find(); $column=blnav(); if(!empty($type)){ $where=array('ModelID'=>$ModelID['ID'],'Child'=>array('neq',0),'Status'=>1,'Edition'=>C('edition')); }else{ $where=array('Child'=>array('neq',0),'Status'=>1,'Edition'=>C('edition')); } if($show=="all"){ !empty($id)?$where['ID']=$id:''; }else if($show=="now"){ $where['ID']=$isid; }else if($show=='now_all'){ $gid=$_GET['cid']; $data=$db->field('path')->where('ID='.$gid.'')->find(); !empty($data['path'])?$path=explode(',', $data['path'])[0]:$gid; $where['ID']=$path; } $pid=$db->where($where)->order('depth')->find(); empty($pid['ID'])?$pid['ID']=I('cid'):$pid['ID']; import("Class.expand",APP_PATH); $object = new \Expand(); $column=$object->erwei2($column,'',$pid['ID']); echo classNavv($column); ?>		
        </div>
        <!-- wap 左侧 end -->

        <div class="ny-right">
            <p class="all"><em><?php $isid=""; $id=!empty($isid)?$isid:I("get.cid");$desclen="";$data=M("class")->where("ID=".$id)->find();$cname["src"]="/Upload".$data["Class_pic"];$cname["name"]=!myempty($data["Name"])?$data["Name"]:"产品展示";$cname["desc"]=substr_cut($data["Class_info"],$desclen);$cname["remark"]=$data["Remark"]; echo ($cname["name"]); ?></em> 共找到 <b><?php $pronum["num"]="2"; echo ($pronum["num"]); ?></b> 件产品</p>
            <ul class="ny-prolist">
                <?php $where='';$db=M("product");$where['path']=array('like',"%".I('get.cid')."%");$where['Title']=array('like',"%".I('title')."%");$where['Edition']=C('EDITION');$proTotalList=$db->where($where)->count();$productListNum=12; $page=Fpage($proTotalList,$productListNum); if(!myempty(I('title'))){ $page->parameter['title']=I('title'); } if(!myempty(I('cid'))){ $page->parameter['cid']=I('cid'); } $num=$proTotalList; $pageshow=$page->show(); $where['path']=array('like',"%".I('get.cid')."%"); $where['Title']=array('like',"%".I('title')."%"); $where['Edition']=C('EDITION'); $data=M('product') ->where($where) ->limit("$page->firstRow,$page->listRows") ->order('Sort DESC,ID DESC') ->select(); $key="<p style='text-align:center;font-size:14px;margin:20px 0px 0px 0px'>对不起,暂无的信息!</p>"; if(empty($data)){ echo $key; } foreach ($data as $productList) : $piclist=explode("###",$productList['Thumbnail'])[$productList['Cover']]; $picpath='Upload'.$piclist; if(!empty($piclist) && file_exists($picpath)){ $productList["src"]="/".$picpath; }else{ $productList["src"]='/Common/images/not-pic.jpg'; } $productList["title"]=substr_cut($productList["Title"],10); $productList["desc"]=substr_cut($productList["Description"],20); $productList["keyword"]=substr_cut($productList["Keyword"],20); $productList["link"]=get_url($productList['ClassID'],$productList["ID"]); ?><li>
                    <div class="pic">
                        <a href="<?php echo ($productList["link"]); ?>"><img src="<?php echo ($productList["src"]); ?>" height="182" width="230" alt="<?php echo ($productList["title"]); ?>" title="<?php echo ($productList["title"]); ?>"></a>
                    </div>
                    <a href="<?php echo ($productList["link"]); ?>" class="name"><?php echo ($productList["title"]); ?></a>
                    <ul class="btn">
                        <li><a href="/aboutt244.html">免费询价</a></li>
                        <li><a href="http://www.tmall.com/">网购询价</a></li>
                    </ul>
                </li><?php endforeach; ?>
            </ul>
            <div class="cl"></div>
