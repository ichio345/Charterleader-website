 ...
 *
 * $jqId: jquery.validate.js 6243 2009-02-19 11:40:49Z joern.zaefferer $jq
 *
...
$jq.format = function(source, params) {
	if ( arguments.length == 1 ) 
		return function() {
			var args = $jq.makeArray(arguments);
			args.unshift(source);
			return $jq.format.apply( this, args );
		};
...
$jq.validator = function( options, form ) {
	this.settings = $jq.extend( {}, $jq.validator.defaults, options );
...
$jq.extend($jq.validator, {
	...
	messages: {
		...
		maxlength: $jq.format("Please enter no more than {0} characters."),
		minlength: $jq.format("Please enter at least {0} characters."),
		rangelength: $jq.format("Please enter a value between {0} and {1} characters long."),
		range: $jq.format("Please enter a value between {0} and {1}."),
		max: $jq.format("Please enter a value less than or equal to {0}."),
		min: $jq.format("Please enter a value greater than or equal to {0}.")
	},
	...
	prototype: {
		...
		focusInvalid: function() {
			if( this.settings.focusInvalid ) {
				try {
					$jq(this.findLastActive() || this.errorList.length && this.errorList[0].element || []).filter(":visible").focus();
				} catch(e) {
		...
		previousValue: function(element) {
			return $jq.data(element, "previousValue") || $jq.data(element, "previousValue", previous = {
				old: null,
				valid: true,
	...
	addMethod: function(name, method, message) {
		...
		$jq.validator.messages[name] = message;
		if (method.length < 3) {
	...
	methods: {
		...
		required: function(value, element, param) {
			...
			switch( element.nodeName.toLowerCase() ) {
			case 'select':
				var options = $jq("option:selected", element);
				return options.length > 0 && ( element.type == "select-multiple" || ($jq.browser.msie && !(options[0].attributes['value'].specified) ? options[0].text : options[0].value).length > 0);
			case 'input':
				if ( this.checkable(element) )
		...
		email: function(value, element) {
			...
			return this.optional(element) || /^((([a-z]|\d|[!#\$jq%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$jq%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$jq/i.test(value);
		},
	
		// http://docs.jquery.com/Plugins/Validation/Methods/url
		url: function(value, element) {
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/iri/
			return this.optional(element) || /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$jq&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$jq&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$jq&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$jq&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$jq&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$jq/i.test(value);
		},
		...
		dateISO: function(value, element) {
			return this.optional(element) || /^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$jq/.test(value);
		},
	
		// http://docs.jquery.com/Plugins/Validation/Methods/dateDE
		dateDE: function(value, element) {
			return this.optional(element) || /^\d\d?\.\d\d?\.\d\d\d?\d?$jq/.test(value);
		},
	
		// http://docs.jquery.com/Plugins/Validation/Methods/number
		number: function(value, element) {
			return this.optional(element) || /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$jq/.test(value);
		},
		...
		digits: function(value, element) {
			return this.optional(element) || /^\d+$jq/.test(value);
		},
		...
		accept: function(value, element, param) {
			...
			return this.optional(element) || value.match(new RegExp(".(" + param + ")$jq", "i")); 
		},
		
		// http://docs.jquery.com/Plugins/Validation/Methods/equalTo
		equalTo: function(value, element, param) {
			return value == $jq(param).val();
		}
...
;(function($jq) {
	$jq.each({
		...
		$jq.event.special[fix] = {
			setup:function() {
				if ( $jq.browser.msie ) return false;
				this.addEventListener( original, $jq.event.special[fix].handler, true );
			},
			teardown:function() {
				if ( $jq.browser.msie ) return false;
				this.removeEventListener( original,
				$jq.event.special[fix].handler, true );
			},
			handler: function(e) {
	...
	$jq.extend($jq.fn, {
		...
		triggerEvent: function(type, target) {
			return this.triggerHandler(type, [$jq.event.fix({ type: type, target: target })]);
		}
	})
})($jq);
